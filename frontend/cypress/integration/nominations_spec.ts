...
describe('Nomination stage', () => {
    describe('Adding and deleting users, finishing nomination', () => {
        ...
        it('A participant that is added is part of the evaluation', () => {
            // To be added
        })
        ...
        it('A participant that is deleted is no longer part of evaluation', () => {
            ...
            nominationPage.deletePersonButton(userToDelete.user).click()
            cy.visitEvaluation(seed.evaluationId, p.user)
            nominationPage.assertParticipantPresent(p.user)
            nominationPage.assertParticipantAbsent(userToDelete.user)
        })

        it('When Facilitator progresses evaluation it moves to next stage', () => {
            // To be added
        })
    })
})
...
function createSeed(progression: Progression = Progression.Nomination) {
    ...
    let facilitator = new Participant({ user: users[0], role: Role.Facilitator, progression: progression })
    let otherFacilitator = new Participant({ user: users[1], role: Role.Facilitator, progression: progression })
    let participant = new Participant({ user: users[2], role: Role.Participant, progression: progression })
    let lead = new Participant({ user: users[3], role: Role.OrganizationLead, progression: progression })
    const seed = new EvaluationSeed({
        progression: progression,
    })
    seed.addParticipant(facilitator).addParticipant(otherFacilitator).addParticipant(participant).addParticipant(lead)
    return seed
}
...
function verifyUserManagementCapabilities(
    ...
    nominationPage.deletePersonButton(participant.user).should('not.exist')
    allParticipants.forEach(p => {
        if (p === participant) {
            return
        }
        if (deleteUser) {
            nominationPage.deletePersonButton(p.user).find('button').should('be.enabled')
        } else {
            nominationPage.deletePersonButton(p.user).find('button').should('be.disabled')
        }
    })
}
