...
namespace scripts
{
    class DbHandler
    {
        ...
        public void OrderQuestionsInEvaluation(string evaluationId)
        {
            ...
            List<Question> questions = context.Questions.Where(q => q.Evaluation.Id.Equals(evaluationId)).ToList();
        ...
        public void OrderQuestionTemplates(string questionFile)
        {
            ...
            foreach (QuestionTemplate questionTemplate in questions)
            {
                ...
                questionTemplate.Order = qtFromFile.Order;
            }

            foreach (QuestionTemplate qt in questions)
            {
                Console.WriteLine($"Foreach question template, order: {qt.Order}, barrier: {qt.Barrier}, text: {qt.Text}");
            }
            List<int> ints = questions.Select(qt => qt.Order).ToList();
            HashSet<int> intSet = new HashSet<int>(ints);
            Console.WriteLine($"Distinct orders after: {intSet}");
